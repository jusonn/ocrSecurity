...
def build_model(alphabet, height, width, color, filters, rnn_units, dropout,
    ...
    inputs = keras.layers.Input((height, width, 3 if color else 1), batch_size=4)
    ...
    x = keras.layers.MaxPooling2D(pool_size=(3,  3), name='maxpool_3')(x)
    ...
    x = keras.layers.MaxPooling2D(pool_size=(3, 3), name='maxpool_5')(x)
    ...
    stn_input_layer = keras.layers.Input(stn_input_output_shape, batch_size=4)
...
if __name__ == '__main__':
    ...
    model = build_model(1,512, 512, 3, 7, 2, 0.1, 2, 4)
    # model.summary()
